import numpy as np
import utils.event_model as em
import dataclasses
...
class SimpleGenerator:
    ...
    def generate_random_primary_vertices(self, n_events, sigma):
        ...
        for _ in range(n_events):
            x = self.rng.normal(0, sigma[0]) #70microns
            y = self.rng.normal(0, sigma[1]) #70 microns
            z = self.rng.normal(0, sigma[2]) #5cm?
        ...
        for event_index in range(n_events):
            ...
            for track_id in range(n_particles):
                ...
                for idx, (module_id, zm, lx, ly) in enumerate(
                    ...
                    if np.abs(x_hit) < lx / 2 and np.abs(y_hit) < ly / 2: # ToDo: Need to account for the rectangular void in the buld!
